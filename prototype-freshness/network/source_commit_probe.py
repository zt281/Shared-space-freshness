#!/usr/bin/env python3
"""Fixed-input comparison of synchronous, one-record writer and batch writer modes."""
import argparse
...
def main():
    ...
    manifest = {'started_utc': datetime.now(timezone.utc).isoformat(), 'worker': str(suite.worker),
                ...
                'clock_ticks_per_second': os.sysconf('SC_CLK_TCK'), 'mode': 'source-commit-smoke' if args.smoke else 'source-commit-matrix',
        ...
        for repeat in range(1 if args.smoke else 3):
            ...
            for rate, duration, burst in ([(40, 2.0, 1)] if args.smoke else [(40, 2.0, 1), (80, 1.0, 8)]):
                for mode in order:
                    ...
                    settings = {'TYCHE_QUIC_SOURCE_MODE': mode, 'TYCHE_QUIC_SOURCE_CAPACITY': '64',
                                'TYCHE_QUIC_SOURCE_WAIT_NS': '2000000' if mode == 'batch' else '0'}
                    result = scenario(suite, name, profile, True, rate=rate, duration=duration, source_settings=settings, burst=burst)

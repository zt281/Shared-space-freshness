...
class OrderedConsumer {
  friend class DurableConsumer; // Restores only a validated result/progress checkpoint.
  ReadableSpace& space;
  Consumer policy;
  ...
  std::uint64_t observed_authority = 1;

  View assess(const Changes& changes, Tick now) {
    const auto backlog = changes.head >= cursor ? changes.head - cursor : 0;
    if (changes.latest.authority != observed_authority || !changes.complete ||
        !changes.latest.usable() || backlog >= lag_limit || processing_failed) {
      if (!recovering) { ++alerts; ++cancellation_attempts; }
      recovering = true;
    }
    observed_authority = changes.latest.authority;
...
public:
  ...
  explicit OrderedConsumer(ReadableSpace& input, std::uint64_t threshold = 4, bool startup_reconciled = true)
      : space(input), policy(startup_reconciled), lag_limit(threshold) {
    require(threshold > 0, "positive demo lag threshold");
  }

...
def main():
    ...
    parser.add_argument('--series', choices=['stage', 'source-commit', 'remote-commit', 'remote-combo'], default='stage')
    args = parser.parse_args()
    evidence = CACHE / (args.series + '-build-evidence') / (args.configuration + '-' + uuid.uuid4().hex[:12])
    ...
    build = CACHE / (args.series + '-' + args.configuration)

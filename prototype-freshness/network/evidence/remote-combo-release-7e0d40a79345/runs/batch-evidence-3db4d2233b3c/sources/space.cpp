...
namespace {
constexpr std::uint64_t named_magic = 0x5459434845535043ULL, named_layout = 3;
...
bool journal_io(int fd, Change& event, bool writing, int crash = -1, std::uint64_t identity = 0) {
  ...
  std::size_t done = 0;
  if (writing) diagnostic::mark("source_write_begin", identity, event.record.epoch, event.sequence);
  if (writing && crash == 1) {
  ...
  if (crash == 2) _exit(77);
  diagnostic::mark("source_written", identity, event.record.epoch, event.sequence);
  int result;
  diagnostic::mark("source_sync_begin", identity, event.record.epoch, event.sequence);
  do { result = fdatasync(fd); } while (result < 0 && errno == EINTR);
  diagnostic::mark("source_sync_end", identity, event.record.epoch, event.sequence, result == 0);
...
bool Space::publish(Tick now) {
  diagnostic::mark("source_publish_enter");
  auto writer = serialize_writer();
  diagnostic::mark("source_writer_acquired");
  Guard lock(*this);
  check_writer();
  if (!prepared || draft.epoch != shared->authoritative_epoch ||
    ...
    return false;
  diagnostic::mark("source_locked", shared->identity, draft.epoch, shared->head + 1);
  draft.received_time = now;
  if (!append(draft)) return false;
  install(draft);
  prepared = false;
  diagnostic::mark("source_published", shared->identity, draft.epoch, shared->head);
  return true;
}
...
void Space::reset_lock_metrics() { lock_calls = 0; lock_wait_ns = 0; lock_max_ns = 0; }
...
Space::BatchResult Space::publish_admitted_batch(const std::vector<Change>& events) {
  ...
  std::uint64_t old_head = 0;
  {
    ...
    for (std::size_t i = 0; i < events.size(); ++i) {
      ...
      diagnostic::mark("source_locked", shared->identity, e.record.epoch, e.sequence, events.size());
    }
  }
  ...
  for (const auto& e : events) diagnostic::mark("source_written", shared->identity, e.record.epoch, e.sequence);
  ...
  for (const auto& e : events) diagnostic::mark("source_sync_begin", shared->identity, e.record.epoch, e.sequence);
  int synced = -1;
  ...
  for (const auto& e : events) diagnostic::mark("source_sync_end", shared->identity, e.record.epoch, e.sequence, 1, events.size());
  if (batch_crash_point == 4) _exit(77); // Saved, not yet shared-published.
  {
    ...
    prepared = false;
    for (const auto& e : events) diagnostic::mark("source_published", shared->identity, e.record.epoch, e.sequence, events.size());
...
bool Space::append(const Record& record) {
  ...
  if (event.sequence > shared->journal_limit || !journal_io(journal, event, true, crash_point, shared->identity)) {
    shared->record_failed = true;

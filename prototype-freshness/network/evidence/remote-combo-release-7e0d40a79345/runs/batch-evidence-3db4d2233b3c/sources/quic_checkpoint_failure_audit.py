...
for state in [old,next_state]:
    ...
    assert state[7] == (prefix[-1][-1] if prefix else 0) and state[8] == sum(e[8] for e in prefix)
    assert state[9] == sum(e[7] for e in prefix) and state[10] == seal([e[-1] for e in prefix])
rows = [json.loads(line) for p in case.glob('*.jsonl') for line in p.read_text().splitlines()]
...
report = {'source_records':len(history), 'checkpoint_revision':old[4], 'checkpoint_cursor':old[5],
          ...
          'errno_recorded':'errno=' in failures[0]['checkpoint']['error'], 'cause_established':False, 'pending_promoted_by_audit':False,

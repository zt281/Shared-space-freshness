...
struct App {
  ...
  // Experimental remote modes: legacy synchronous accept, per-log saver thread, per-log batch saver.
  // The application thread owns received/durable/visible, gap policy, storage_failed and progress
  // replies; saver threads only run pwrite + fdatasync + the bounded shared-lock install inside
  // ReplicaSpace and report completions back as ordinary application jobs.
  std::string remote_mode="sync";
  std::size_t remote_capacity=64,remote_batch=1;
  std::uint64_t remote_wait_ns=0,overflows=0;
  std::array<std::unique_ptr<RemoteBatchSaver>,2> savers;
  std::array<Gate,2> remote_gates;
  ...
  App(bool server,const std::filesystem::path&dir,std::string t,bool create,std::uint16_t p,
    ...
    if(source){
      ...
      if(const auto*v=std::getenv("TYCHE_QUIC_REMOTE_MODE"))remote_mode=v;
      require(remote_mode=="sync"||remote_mode=="worker"||remote_mode=="batch","remote mode sync|worker|batch");
      if(const auto*v=std::getenv("TYCHE_QUIC_REMOTE_CAPACITY"))remote_capacity=std::stoull(v);
      require(remote_capacity>0&&remote_capacity<=64,"remote capacity 1..64 per log");
      remote_batch=remote_mode=="batch"?std::min<std::size_t>(8,remote_capacity):1;
      remote_wait_ns=remote_mode=="batch"?2000000:0;
      if(const auto*v=std::getenv("TYCHE_QUIC_REMOTE_WAIT_NS"))remote_wait_ns=std::stoull(v);
      require(remote_wait_ns<=1000000000ULL,"remote batch wait at most one second");
      require(remote_mode=="batch"||remote_wait_ns==0,"single-record remote mode has no batch wait");
    }
    for(int f=1;f<=2;++f){
      if(source){
        ...
        if(remote_mode!="sync"){
          const auto index=f-1;
          savers[index]=std::make_unique<RemoteBatchSaver>(*replicas[index],remote_capacity,remote_batch,remote_wait_ns,
            [this,index](std::vector<RemoteBatchSaver::Done> items,std::string error){
              // Saver thread: report completion as an application job; never touch connections here.
              while(!stopping.load()&&!finish_worker.load()){
                if(enqueue([this,index,items,error]{saver_done(index,items,error);}))return;
                std::this_thread::sleep_for(std::chrono::milliseconds(1));
              }
            });
          auto&g=remote_gates[index];
          require(pipe2(g.notify,O_CLOEXEC)==0&&pipe2(g.release,O_CLOEXEC)==0,"remote fault pipes");
          require(fcntl(g.release[1],F_SETFL,O_NONBLOCK)==0,"nonblocking remote gate release");
        }
      }
    }
    ...
    log("started","\"source\":"+std::string(source?"true":"false")+",\"port\":"+std::to_string(port)+",\"source_mode\":"+quoted(source_mode)+",\"remote_mode\":"+quoted(remote_mode));
  }
  void connect(){
  ...
  void subscribe(std::uint64_t selected){
    ...
    for(int i=0;i<2;++i){durable[i]=visible[i]=received[i]=replicas[i]->attachment().head;if(savers[i])savers[i]->reset();}
  ...
  void message(Stream&s,const Words&w){
    ...
    if(savers[i]){
      auto&saver=*savers[i];
      RemoteBatchSaver::Admission admission;
      try{admission=saver.admit(event,generation);}
      catch(...){storage_failed=true;restrict_all();throw;}
      if(admission==RemoteBatchSaver::Admission::gap){++gaps;log("gap","\"flow\":"+std::to_string(i+1));subscribe(mask);return;}
      if(admission==RemoteBatchSaver::Admission::full){
        // Bounded save queue overflow is temporary pressure, not a save failure: restrict the
        // affected object (no damaged latch) and close the shared replication connection;
        // the admitted prefix keeps draining and a reconnect replays from the durable cursor.
        ++overflows;
        replicas[i]->restrict(false);
        log("remote_overflow","\"flow\":"+std::to_string(i+1)+",\"R\":"+std::to_string(received[i])+",\"sequence\":"+std::to_string(event.sequence));
        shutdown(c,9);
        return;
      }
      received[i]=std::max(received[i],event.sequence);
      if(admission==RemoteBatchSaver::Admission::duplicate){
        ++duplicates;
      ...
      return; // Fresh admissions advance D/V and reply progress only at saver completion.
    }
  ...
  void saver_done(int i,const std::vector<RemoteBatchSaver::Done>&items,const std::string&error){
    // Application worker only; saver threads never touch business state or connections.
    if(!error.empty()){
      // Remote save failure keeps the permanent damaged/blocked semantics; no automatic repair.
      storage_failed=true;restrict_all();
      if(active)fail(*active,"remote save: "+error);
      return;
    }
    durable[i]=visible[i]=replicas[i]->attachment().head;
    if(!stopping&&active&&active->connected&&!active->closing)
      for(const auto&item:items)if(active->generation==item.generation)
        control(*active,Words{protocol,4,generation,static_cast<std::uint64_t>(i+1),static_cast<std::uint64_t>(101+i),received[i],durable[i],visible[i],2});
    maybe_ready();
    log("replica_progress","\"g\":"+std::to_string(generation)+",\"flow\":"+std::to_string(i+1)+",\"R\":"+std::to_string(received[i])+",\"D\":"+std::to_string(durable[i])+",\"V\":"+std::to_string(visible[i])+",\"batch\":"+std::to_string(items.size()));
  }
  ...
  void admit(std::uint64_t request,int flow,int count,Tick ingress){
    ...
    if(publications.size()>=2*source_capacity){
      publishers[i]->reject_input(count);restrict_source(i);
      log("command_error","\"request\":"+std::to_string(request)+",\"requested\":"+std::to_string(count)+
          ",\"unattempted\":"+std::to_string(count)+",\"receipts\":[],\"reason\":\"completion capacity exhausted; required input rejected\"");
      return;
    }
...
#undef DIAG_FIELD
  ...
  void status(std::uint64_t request){
    ...
    out<<"],\"remote_mode\":"<<quoted(remote_mode)<<",\"overflows\":"<<overflows<<",\"savers\":[";
    {bool comma=false;
    for(int i=0;i<2;++i)if(savers[i]){if(comma)out<<',';comma=true;auto m=savers[i]->metrics();pollfd gate{remote_gates[i].notify[0],POLLIN,0};poll(&gate,1,0);
      out<<"{\"admitted\":"<<m.admitted<<",\"duplicates\":"<<m.duplicates<<",\"completed\":"<<m.completed<<",\"batches\":"<<m.batches
         <<",\"syncs\":"<<m.syncs<<",\"max_pending\":"<<m.max_pending<<",\"pending\":"<<m.pending<<",\"restricted\":"<<(m.restricted?"true":"false")
         <<",\"at_gate\":"<<((gate.revents&POLLIN)?"true":"false")<<'}';}}
  ...
  void command(const std::string&line){
    ...
    if(op=="remote_release"){int f;require(!source&&bool(in>>f)&&f>=1&&f<=2&&savers[f-1]&&savers[f-1]->gate_armed(),"remote gate armed");
      const char go='G';require(write(remote_gates[f-1].release[1],&go,1)==1,"remote gate release");log("ack","\"request\":"+std::to_string(request));return;}
    if(op=="release_sync"){require(source||remote_mode=="sync","sync gate needs the synchronous remote path");pause_sync=false;log("ack","\"request\":"+std::to_string(request));return;}
    ...
    if(op=="gate_sync"){require(source||remote_mode=="sync","sync gate needs the synchronous remote path");pause_sync=true;log("ack","\"request\":"+std::to_string(request));return;}
    require(enqueue([this,line,request,ingress]{
      ...
      try{
        ...
        else if(action=="arm"){require(!source&&remote_mode=="sync"&&bool(args>>crash)&&crash>=1&&crash<=4,"crash point 1..4");}
        else if(action=="sync_error"){require(!source&&remote_mode=="sync","receiver sync seam");sync_error=true;}
        else if(action=="remote_gate"||action=="remote_arm"||action=="remote_sync_error"){
          int f;require(!source&&bool(args>>f)&&f>=1&&f<=2&&savers[f-1],"idle remote batch fault seam");
          if(action=="remote_gate"){auto&g=remote_gates[f-1];savers[f-1]->arm_gate(g.notify[1],g.release[0]);}
          else if(action=="remote_sync_error")savers[f-1]->arm_sync_error();
          else{int point;require(bool(args>>point)&&point>=0&&point<=6,"remote crash point 0..6");savers[f-1]->arm_crash(point);}
        }
  ...
  void close(){
    ...
    for(int i=0;i<2;++i)if(savers[i]){const char go='G';const ssize_t n=write(remote_gates[i].release[1],&go,1);(void)n;}
    ...
    // Saver threads still drain every admitted record to disk; completion replies are moot once stopping.
    for(int i=0;i<2;++i)if(savers[i]){savers[i]->close();for(auto fd:remote_gates[i].notify)::close(fd);for(auto fd:remote_gates[i].release)::close(fd);}

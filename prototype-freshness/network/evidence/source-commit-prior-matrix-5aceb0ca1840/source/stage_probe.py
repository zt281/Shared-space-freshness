...
def scenario(suite, name, profile, trace, rate=40, duration=2.0, drain=12.0, capacity=65536, source_settings=None, burst=1):
    ...
    def node(role, port):
        return StageProcess(test, role, [suite.worker, role, test.source_root if role == 'source' else test.replica_root,
                            ...
                            environment_overrides=source_settings if role == 'source' else None)

#!/usr/bin/env python3
"""Combined source-batch x remote save-queue comparison; heavy block is a new, non-comparable condition."""
import argparse
...
SOURCE_BATCH = {'TYCHE_QUIC_SOURCE_MODE': 'batch', 'TYCHE_QUIC_SOURCE_CAPACITY': '64',
                'TYCHE_QUIC_SOURCE_WAIT_NS': '2000000'}


def remote_settings(mode):
    return {'TYCHE_QUIC_REMOTE_MODE': mode, 'TYCHE_QUIC_REMOTE_CAPACITY': '64',
...
def main():
    ...
    parser.add_argument('--heavy', action='store_true', help='only the heavier, not directly comparable tiers')
    ...
    manifest = {'started_utc': datetime.now(timezone.utc).isoformat(), 'worker': str(suite.worker),
                ...
                'mode': 'remote-combo-heavy' if args.heavy else ('remote-combo-smoke' if args.smoke else 'remote-combo-matrix'),
    ...
    try:
        ...
        blocks = []
        if args.heavy:
            # New heavier condition: 200/s, 20 records per 100ms, one second. Not comparable
            # with the 18-tier block; overflow/reconnect recovery is recorded, not masked.
            blocks.append(('heavy', 3, [(200, 1.0, 20)]))
        else:
            blocks.append(('main', 1 if args.smoke else 3, [(40, 2.0, 1)] if args.smoke else [(40, 2.0, 1), (80, 1.0, 8)]))
        for label, rounds, loads in blocks:
            for repeat in range(rounds):
                order = modes[repeat:] + modes[:repeat]
                for rate, duration, burst in loads:
                    for mode in order:
                        name = f'{repeat + 1}-{rate}ps-burst-{burst}-{mode}-trace-1'
                        if label == 'heavy':
                            name = 'heavy-' + name
                        result = scenario(suite, name, profile, True, rate=rate, duration=duration,
                                          source_settings=SOURCE_BATCH, remote_settings=remote_settings(mode),
                                          burst=burst, reconnect_on_close=True)
                        manifest['results'].append(result)
                        (suite.root / 'manifest.json').write_text(json.dumps(manifest, indent=2))
                        print(json.dumps({'case': name, 'passed': result['passed'],
                                          'completed': result.get('observed_complete'),
                                          'reconnects': result.get('reconnects')}), flush=True)

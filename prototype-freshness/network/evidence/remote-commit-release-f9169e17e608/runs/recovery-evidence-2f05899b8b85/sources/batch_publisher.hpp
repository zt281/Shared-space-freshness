...
public:
  ...
  void reject_input(std::size_t count); // Required inputs rejected by an upstream bounded completion queue.
  ...
  Metrics metrics();
private:
  struct Item { Change event; std::uint64_t enqueued, request; std::promise<Receipt> promise; };
  Space& space;

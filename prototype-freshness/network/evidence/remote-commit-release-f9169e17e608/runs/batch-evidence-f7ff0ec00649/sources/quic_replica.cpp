...
bool ReplicaSpace::identical(std::uint64_t seq,const Change&e){return record_words_of(load(seq))==record_words_of(e);}
std::uint64_t ReplicaSpace::accept_batch(const std::vector<Change>&events,int crash,bool sync_error,const std::function<void()>&before_sync) {
  require(!events.empty()&&events.size()<=8,"bounded remote save batch");
  std::size_t fresh_begin=0;
  {Guard g(*this);
    require(writer&&shared->pid==getpid()&&!shared->damaged,"replica provider restricted by storage/conflict");
    while(fresh_begin<events.size()&&events[fresh_begin].sequence<=shared->head) {
      auto old=load(events[fresh_begin].sequence);
      if(record_words_of(old)!=record_words_of(events[fresh_begin])){shared->damaged=shared->blocked=true;++shared->generation;throw std::runtime_error("conflicting replica duplicate");}
      ++fresh_begin;
    }
    if(fresh_begin==events.size())return 0;
    for(std::size_t k=fresh_begin;k<events.size();++k) {
      const auto&e=events[k];
      require(e.sequence==shared->head+1+(k-fresh_begin),"replica sequence gap");
      require(e.record.epoch>=(k==fresh_begin?shared->record.epoch:events[k-1].record.epoch),"replica epoch regression");
    }
  }
  const auto fresh=events.size()-fresh_begin;
  require(crash!=2||fresh>=2,"half-second-record crash needs a batch of at least two fresh records");
  std::vector<Bytes> encoded;
  for(std::size_t k=fresh_begin;k<events.size();++k) {
    auto words=Words{protocol,log_id};auto rec=record_words_of(events[k]);words.insert(words.end(),rec.begin(),rec.end());
    auto bytes=encode(words);require(bytes.size()==disk_bytes,"replica disk framing");
    encoded.push_back(std::move(bytes));
  }
  try {
    if(crash==0)_exit(77); // Nothing of this batch was written.
    for(std::size_t k=0;k<encoded.size();++k) {
      const auto&e=events[fresh_begin+k];
      const auto at=static_cast<off_t>((e.sequence-1)*disk_bytes);
      diagnostic::mark("replica_write_begin",log_id,e.record.epoch,e.sequence);
      if(crash==2&&k==1){io(journal,encoded[k].data(),encoded[k].size()/2,at,true);_exit(77);}
      io(journal,encoded[k].data(),encoded[k].size(),at,true);
      diagnostic::mark("replica_written",log_id,e.record.epoch,e.sequence);
      if(crash==1&&k==0)_exit(77); // Complete first record, rest of the batch absent.
    }
    if(before_sync)before_sync();
    if(crash==3)_exit(77); // Full batch written, sync not yet observed.
    if(sync_error){errno=EIO;check(false,"injected replica fdatasync");}
    for(std::size_t k=fresh_begin;k<events.size();++k){const auto&e=events[k];diagnostic::mark("replica_sync_begin",log_id,e.record.epoch,e.sequence,0,fresh);}
    sync_file(journal);
    for(std::size_t k=fresh_begin;k<events.size();++k){const auto&e=events[k];diagnostic::mark("replica_sync_end",log_id,e.record.epoch,e.sequence,1,fresh);}
    if(crash==4)_exit(77); // Saved, not yet shared-published.
    for(std::size_t k=fresh_begin;k<events.size();++k){const auto&e=events[k];diagnostic::mark("replica_publish_enter",log_id,e.record.epoch,e.sequence);}
    {Guard g(*this);require(events.back().sequence==shared->head+fresh&&!shared->damaged,"replica concurrent provider mutation");
      for(std::size_t k=fresh_begin;k<events.size();++k) {
        const auto&e=events[k];
        diagnostic::mark("replica_publish_locked",log_id,e.record.epoch,e.sequence);
        shared->ring[(e.sequence-1)%shared->ring.size()]=e;shared->record=e.record;
        if(crash==5)_exit(77); // Torn snapshot: ring/record ahead of head under the robust mutex.
        diagnostic::mark("replica_published",log_id,e.record.epoch,e.sequence,fresh);
      }
      shared->head=events.back().sequence;
    }
    if(crash==6)_exit(77); // Published; the application thread never learns of it.
    return events.back().sequence;
  }catch(...){restrict(true);throw;}
}
}

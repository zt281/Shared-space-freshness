...
void DurableConsumer::save(State next, bool create) {
  diagnostic::mark("checkpoint_begin",next.space,0,next.cursor);
  ...
  diagnostic::mark("checkpoint_opened",next.space,0,next.cursor,next.revision);
  if (crash_point == 1) {
    transfer(file.fd, &next, sizeof(next) / 2, true);
    _exit(77);
  }
  transfer(file.fd, &next, sizeof(next), true);
  diagnostic::mark("checkpoint_written",next.space,0,next.cursor);
  trigger(2);
  diagnostic::mark("checkpoint_sync_begin",next.space,0,next.cursor);
  sync_file(file.fd);
  diagnostic::mark("checkpoint_sync_end",next.space,0,next.cursor);
  trigger(3);
  diagnostic::mark("checkpoint_replace_begin",next.space,0,next.cursor);
  if (create) {
  ...
  diagnostic::mark("checkpoint_replaced",next.space,0,next.cursor);
  trigger(4);
  diagnostic::mark("checkpoint_dirsync_begin",next.space,0,next.cursor);
  sync_file(directory, true);
  diagnostic::mark("checkpoint_dirsync_end",next.space,0,next.cursor);
  trigger(5);
  state = next;
  diagnostic::mark("checkpoint_saved",next.space,0,next.cursor,next.revision);
}
...
bool DurableConsumer::consume(Tick now, std::size_t budget, std::vector<Change>& processed) {
  ...
  return ordered.consume(now, budget, [&](const Change& event) {
    diagnostic::mark("consumer_begin",state.space,event.record.epoch,event.sequence);
    auto next = state;
    ...
    diagnostic::mark("consumer_calculated",state.space,event.record.epoch,event.sequence);
    ...
    diagnostic::mark("consumer_completed",state.space,event.record.epoch,event.sequence);
    return true;
  });
}

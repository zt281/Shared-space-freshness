...
def summarize(root, config, analysis_root=None, allow_worker_errors=False):
    ...
    for name in names:
        ...
        assert stats["sequence_errors"] == stats["payload_errors"] == 0, (name, stats)
        if not allow_worker_errors:
            assert stats["error"] == "", (name, stats)
        ...
        result = {"planned": len(records), "completed": len(completed), "missing_at_exit": not_complete,
                  ...
                  "not_started_at_exit": sum(r["started_ns"] == 0 for r in records) if not stats["error"] else None,
                  "rows_without_start_timestamp": sum(r["started_ns"] == 0 for r in records),
    ...
    return {"config": config, "audit_passed": True, "audit_scope": "saved data consistency, not operational or performance success",
            "all_workers_healthy": all(not results[n]["resource"]["error"] for n in names), "processes": results,

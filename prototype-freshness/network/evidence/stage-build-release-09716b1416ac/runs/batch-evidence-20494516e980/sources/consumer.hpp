...
public:
  ...
  void observe(const View& v, Tick now) {
    ...
    if (v.provider_generation != provider_generation && intent.result == "pending")
      intent.result = "not_ready";
    if (v.authority != epoch || v.provider_generation != provider_generation || (!fresh && last_usable)) {
      ++recovery;
      reconciled = false;
      ready_at = -1;
      epoch = v.authority;
      provider_generation = v.provider_generation;
    }

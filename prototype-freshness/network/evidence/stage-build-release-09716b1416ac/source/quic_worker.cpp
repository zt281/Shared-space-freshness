...
struct App {
  ...
  void run(){
    while(!finish_worker){
      ...
      try{if(job.function){diagnostic::mark("job_started",0,0,0,job.id,job.enqueued_ns);job.function();diagnostic::mark("job_finished",0,0,0,job.id);}pump();}catch(const std::exception&e){failure=e.what();log("worker_error","\"reason\":"+quoted(failure));if(!source&&active)fail(*active,failure);}
    }
  }
    ...
    require(enqueue([this,line,request]{
      ...
      try{
        ...
        if(action=="connect")connect();
          ...
          for(int k=0;k<n;++k){diagnostic::mark("source_prepare_begin");sources[f-1]->prepare(now_ms());diagnostic::mark("source_prepared");require(sources[f-1]->publish(now_ms()),"source publication rejected");diagnostic::mark("source_publish_returned");}

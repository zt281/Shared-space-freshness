...
class StageProcess(Process):
    def __init__(self, test, label, args, trace, capacity=65536, **kwargs):
        self.envelopes = []
...
def scenario(suite, name, profile, trace, rate=40, duration=2.0, drain=12.0, capacity=65536):
    ...
    try:
        ...
        for index in range(count):
            target = start + int(index * 1e9 / rate)

...
class Process:
    def __init__(self, test, label, args, startup="started", expect_failure=False, environment_overrides=None):
        ...
        if environment_overrides is not None:
            ...
            (test.path / (label + ".environment.json")).write_text(json.dumps(environment_overrides, indent=2))
        self.p = subprocess.Popen(self.args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=self.err, env=environment)
...
class Suite:
    ...
    def run(self, only=None):
        try:
            cases = [("selective-replay-checkpoint", basic)]
            for point in [1, 2, 3, 4]:
                cases.append((f"receiver-crash-{point}", lambda t, p=point: crash_test(t, p)))
            cases += [("save-error", save_error), ("conflicting-duplicate", conflict),
                      ("stream-reset", stream_reset), ("oversize-frame", oversize_frame)]
            for kind in ["wrong-pin", "missing-client-cert", "wrong-ca"]:
                cases.append((kind, lambda t, k=kind: authentication(t, k)))
            cases += [("slow-receiver", slow), ("source-restart", source_restart)]
            assert only is None or only in dict(cases), 'known scenario'
            for name, fn in cases:
                if only is None or only == name:
                    self.scenario(name, fn)
            result = {"passed": True}
...
def slow(t):
    # Isolate receiver pressure from a legacy bulk source call starving the
    # application pump for longer than the unchanged 5-second idle timeout.
    # The original live-input failure remains retained as a source limitation.
    s = t.node("source")
    s.command("publish 1 200", timeout=30)
    r = t.node("receiver")
    r.command("slow 150")
    r.command("connect")
...
if __name__ == "__main__":
    ...
    raise SystemExit(0 if Suite(args.worker, args.evidence_root).run(args.scenario) else 1)

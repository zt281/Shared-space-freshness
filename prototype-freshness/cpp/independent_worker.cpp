...
int main(int argc, char** argv) {
  try {
    require(argc == 6, "usage: independent_worker ROLE NAME JOURNAL IDENTITY ADDRESS");
    ...
    require(role == "create" || role == "reader" || role == "publisher", "unknown process role");
    auto space = std::make_unique<Space>(location, role == "create" ? Space::Access::create :
                                                 role == "reader" ? Space::Access::reader : Space::Access::publisher);
    ...
    auto reply = [&](const std::string& action, Tick now, bool ok, const std::string& outcome,
      ...
      if (consumer) {
        const auto s = consumer->status(now);

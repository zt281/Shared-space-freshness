...
from independent_scenarios import Run, Worker, write_json
...
def main():
    ...
    manifest = {
        ...
        "source_head": subprocess.check_output(["git", "-C", str(cpp), "rev-parse", "HEAD"], text=True).strip(),
    ...
    try:
        ...
        run.check("observed_first_event_processed_again", consumed["events"] == repeated["events"])
        ...
        run.check("observed_replaced_tail_never_delivered", delivered["cursor"] == 2
                  and delivered["events"] == published["events"]

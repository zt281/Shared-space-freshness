...
def old_layout(run, version):
    owner = Worker(run, "creator", "create")
    run.created_names.add(run.name)
    owner.exit()
    region = Path("/dev/shm") / run.name[1:]
    raw = bytearray(region.read_bytes())
    run.check("new_shared_layout_is_explicitly_versioned", struct.unpack_from("<Q", raw, 8)[0] == 3)
    struct.pack_into("<Q", raw, 8, version)
    region.write_bytes(raw)  # Exclusive synthetic region, no attached process remains.
    (run.root / "old-layout-region.bin").write_bytes(raw)
    run.failed_start("old_same_size_layout_rejected", contains="incompatible or uninitialized")
...
def main():
    ...
    cases += [(f"old-layout-{v}", old_layout, v) for v in (1, 2)]
    summary = []
    try:

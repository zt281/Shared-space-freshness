...
class Worker:
    def __init__(self, run, label, role, *, identity=None, name=None, journal=None, address=0,
                 checkpoint=None, consumer_id=1, startup_tick=0):
        self.run, self.label = run, label
        self.interactive = role in {"create", "reader", "publisher", "durable", "durable_create"}
        self.events = []
        ...
        if checkpoint is not None:
            command += [str(checkpoint), str(consumer_id), str(startup_tick)]
...
def scenarios(run):
    ...
    replacement.send("prepare", 1421)  # A real unpublished old-epoch private draft dies with this process.
    ...
    next_writer.send("prepare", 1440)
    check("takeover_requires_source_snapshot_before_ordinary_publish",
          not next_writer.send("publish", 1440, expect_ok=False)["ok"])
...
def main():
    ...
    source_files = sorted(set(source.glob("*.hpp")) | set(source.glob("*.cpp")) |
                          set(source.glob("*.cmake")) | set(source.glob("*.sh")) |
                          {Path(__file__).resolve(), source / "CMakeLists.txt"})
    source_archive = run.root / "sources"
    source_archive.mkdir()
    for path in source_files:
        shutil.copyfile(path, source_archive / path.name)

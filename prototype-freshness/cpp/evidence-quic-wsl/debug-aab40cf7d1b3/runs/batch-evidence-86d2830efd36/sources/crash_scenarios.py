...
def corrupt_log(run, mode):
    ...
    if mode == "short":
        ...
        if mode == "boolean":
            changed[80] = 2  # Invalid bool representation, before any typed bool read.
        else:
...
def checkpoint_failures(run):
    ...
    for mode in ("partial", "checksum", "ahead", "history", "space"):
        ...
        run.failed_start("checkpoint_" + mode + "_rejected", role="durable", checkpoint=bad_path, consumer_id=43,
                         contains={"partial": "length", "checksum": "invalid checkpoint", "ahead": "ahead",
                                   "history": "does not match", "space": "identity mismatch"}[mode])
...
def main():
    ...
    cases += [("corrupt-" + mode, corrupt_log, mode) for mode in ("short", "sequence", "seal", "boolean")]

...
Changes ReplicaSpace::read_after(std::uint64_t cursor,std::size_t budget,Tick now) {
  Guard g(*this);Changes out{view(now),shared->head,{},0,!shared->damaged};
  ...
  for(auto seq=cursor+1;seq<=out.head&&out.events.size()<budget;++seq) {
    try {
      ...
      else {e=load(seq);++out.from_journal;}
...
bool ReplicaSpace::accept(const Change&e,int crash,bool sync_error,const std::function<void()>&before_sync) {
  ...
  try {
    ...
    sync_file(journal);if(crash==3)_exit(77);
    {Guard g(*this);require(e.sequence==shared->head+1&&!shared->damaged,"replica concurrent provider mutation");install(e);}
    if(crash==4)_exit(77);
    return true;

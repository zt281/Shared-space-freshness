...
int main(int argc, char** argv) {
  try {
    ...
    std::vector<Change> proposals;
    std::vector<BatchPublisher::Submission> submissions;
    std::vector<BatchPublisher::Receipt> receipts;
    std::future<void> legacy_write;
    int gate_notify[2]{-1, -1}, gate_release[2]{-1, -1};
    ...
    auto reply = [&](const std::string& action, Tick now, bool ok, const std::string& outcome,
      ...
      if (batch) {
        const auto totals = batch->metrics();
        std::cout << ",\"batch\":{\"accepted\":" << totals.accepted << ",\"rejected\":" << totals.rejected
                  << ",\"committed\":" << totals.committed << ",\"unknown\":" << totals.unknown
                  << ",\"batches\":" << totals.batches << ",\"syncs\":" << totals.syncs
                  << ",\"max_pending\":" << totals.max_pending << "},\"proposals\":";
        events_json(proposals, now);
        std::cout << ",\"receipts\":[";
        for (std::size_t i = 0; i < receipts.size(); ++i) {
          const auto& r = receipts[i];
          if (i) std::cout << ',';
          std::cout << "{\"sequence\":" << r.proposed.sequence << ",\"seal\":" << r.proposed.seal
                    << ",\"accepted\":" << (r.accepted ? "true" : "false")
                    << ",\"state\":" << static_cast<int>(r.batch.state)
                    << ",\"sync_ns\":" << r.batch.synced_ns
                    << ",\"published_ns\":" << r.batch.published_ns
                    << ",\"confirmed_by_ns\":" << r.confirmed_by_ns << '}';
        }
        std::cout << ']';
      }
      std::cout << '}' << std::endl;
    ...
    while (std::getline(std::cin, line)) {
      ...
      try {
        ...
        else if (action == "arm_batch_crash") experiment::Faults::batch_crash(*space, static_cast<int>(budget));
        else if (action == "arm_batch_sync_failure") experiment::Faults::batch_sync_failure(*space);
        else if (action == "batch_open") {
          require(!batch, "one queue per worker");
          batch = std::make_unique<BatchPublisher>(*space, budget, std::min<std::size_t>(Space::buffer_capacity, budget), now * 1000000ULL);
        }
        else if (action == "batch_plan") {
          require(batch && proposals.empty(), "one explicit proposal set");
          const auto seed = batch->seed();
          for (std::size_t i = 0; i < budget; ++i)
            proposals.push_back(Space::proposal(seed.sequence + i + 1,
              complete_record(seed.record.epoch, seed.record.version + i + 1, now)));
        }
        else if (action == "batch_send") {
          require(batch && submissions.empty(), "submit originals once; unknown is never resent");
          for (const auto& proposal : proposals) submissions.push_back(batch->submit(proposal.record));
        }
        else if (action == "batch_drain") {
          require(batch != nullptr, "batch queue required");
          batch->close();
          for (auto& submission : submissions) if (submission.final.valid()) {
            receipts.push_back(submission.final.get());
            if (receipts.back().batch.state != Space::BatchState::committed)
              require(!space->read(now).usable(), "failed receipt must observe shared restriction");
          }
        }
        else if (action == "batch_gate") {
          require(pipe(gate_notify) == 0 && pipe(gate_release) == 0, "batch gate pipes");
          experiment::Faults::batch_gate(*space, gate_notify[1], gate_release[0]);
        }
        else if (action == "batch_wait") {
          char value = 0;
          require(::read(gate_notify[0], &value, 1) == 1 && value == 'S', "batch at sync gate");
        }
        else if (action == "batch_release") {
          require(write(gate_release[1], "G", 1) == 1, "release batch sync gate");
        }
        else if (action == "batch_legacy_start") {
          legacy_write = std::async(std::launch::async, [&] { space->verify(150); });
        }
        else if (action == "batch_legacy_join") { legacy_write.get(); }

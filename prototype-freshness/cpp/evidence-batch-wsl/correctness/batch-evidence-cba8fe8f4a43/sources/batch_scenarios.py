...
def publication(run, point):
    ...
    for item in proposed:
        queried += replacement.send("query", 100, item["sequence"])["events"]
...
def gate_and_serialization(run, _):
    ...
    run.check("writer_control_plane_reads_while_sync_is_blocked", held["head"] == 1 and held["view"]["version"] == 1)
    run.check("complete_pending_records_already_written_but_not_published", len(events(run.journal)) == 5)
    observed = reader.send("consume", 100, 8)
    run.check("independent_reader_gets_only_old_complete_event_during_disk_gate",
              observed["head"] == 1 and [e["sequence"] for e in observed["events"]] == [1]
              and observed["view"]["version"] == 1 and observed["view"]["validity"] == "valid")
    ...
    run.check("legacy_append_waits_on_write_lock_without_blocking_reader",
              reader.send("inspect", 100)["head"] == 1 and len(events(run.journal)) == 5)
    ...
    run.check("batch_then_legacy_append_have_no_slot_overwrite", final_read["head"] == 6
              and canonical(reader.events[1:5]) == canonical(proposed)
              and reader.events[-1]["sequence"] == 6 and reader.events[-1]["record"]["version"] == 5)
    run.check("no_half_snapshot_after_batch_release", final_read["view"]["version"] == 5
              and final_read["view"]["proof"] == 6 and final_read["view"]["validity"] == "valid")
...
def main():
    ...
    cases += [("sync-gate-and-serialization", gate_and_serialization, None),
              ("queue-overflow", overflow, None), ("sync-failure", sync_failure, None), ("batch-order", ordered, None)]
    summary = []
    try:

...
def summarize(root, config):
    ...
    results, all_delays, operation_costs = {}, [], []
...
def main():
    ...
    def launch(role, index):
        label = "publisher" if role == "publisher" else f"consumer-{index}"
    ...
    try:
        ...
        print(json.dumps({"root": str(root), "rate": args.rate, "fanout": args.fanout, "mode": args.mode,
                          "publisher": result["processes"]["publisher"], "aggregate": result["aggregate"]}), flush=True)

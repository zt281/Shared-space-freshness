...
DurableConsumer::DurableConsumer(Space& input, const std::filesystem::path& checkpoint,
...
void DurableConsumer::save(State next, bool create) {
  ...
  } else require(rename(temporary.c_str(), path.c_str()) == 0, "replace complete checkpoint");
  trigger(4);
